...
def process_audio_files(folder_path):
    ...
    for subfolder in os.listdir(folder_path):
        ...
        # Extraer el número de mentas de la carpeta (asumiendo que el nombre es numérico)
        try:
        ...
        for file in audio_files:
            ...
            freq_list.append(peak_frequencies)
        
        # Apilamiento: promedio de las curvas de frecuencia pico
        min_length = min(map(len, freq_list))
        stacked_frequencies = np.mean([f[:min_length] for f in freq_list], axis=0)
        stacked_times = times_list[0][:min_length]  # Todas las grabaciones tienen los mismos tiempos
        
        results[num_mints] = {"times": stacked_times, "frequencies": stacked_frequencies}
        overall_avg_frequencies[num_mints] = np.mean(avg_frequencies)
    
    return results, overall_avg_frequencies
...
filtered_data = {m: v for m, v in data.items() if min_mints <= m <= max_mints}

# Mostrar la frecuencia media por cantidad de mentas en el rango seleccionado
print("\nFrecuencia media por cantidad de mentas en el rango seleccionado:")
for mints, freq in overall_avg.items():
    if min_mints <= mints <= max_mints:
        print(f"{mints}: {freq:.2f} Hz")

# Graficar las señales filtradas
plt.figure(figsize=(10, 5))
for mints, values in filtered_data.items():
    plt.plot(values["times"], values["frequencies"], marker="o", linestyle="-", markersize=2, linewidth=0.8, alpha=0.8, label=f"{mints} mentas")

plt.xlabel("Tiempo (s)")
plt.ylabel("Frecuencia Pico (Hz)")
plt.title(f"Comparación de Picos de Frecuencia Apilados ({min_mints}-{max_mints} mentas)")
plt.legend()
plt.grid(True)
plt.show()
